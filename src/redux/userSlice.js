...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    // Keep a synchronous addSkill for compatibility (Resume.jsx or other components that import addSkill)
    addSkill: (state, action) => {
      // action.payload should be a skill object
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteSkillBackend.rejected, (state, action) => {
        state.skillStatus = 'failed';
        state.error = action.payload || action.error?.message;
      })
      ...
      .addCase(deleteProjectFromBackend.rejected, (state, action) => {
        state.projectStatus = 'failed';
        state.error = action.payload || action.error?.message;
      })
      ...
      .addCase(fetchLeetCodeStats.rejected, (state, action) => {
...
// Export synchronous actions (including addSkill for compatibility)
export const {
  logout,
  updateLocalPhoto,
  addAppliedJobLocal,
  addSkill,         // <-- synchronous addSkill action (fixes your build error)
  updateSemester
} = userSlice.actions;
